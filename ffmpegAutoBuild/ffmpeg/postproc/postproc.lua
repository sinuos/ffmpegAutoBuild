...
--[[
函数:postproc_parser_allfile
功能:postproc 处理函数
参数:
返回:
]]--
function postproc_parser_allfile(szPath,includes,proType,isToc89,useClang)
	local filename="Makefile";
	local projName="libpostproc";
	local makefiletabl={
	...
	for index,value in ipairs(makefiletabl) do
		...
		if(value.tbl~=nil) then
			...
			if isToc89 then
				postproc_file_Replace_c99_to_c98(szPath..value.path,avcodectbl);
			end;
		end;
	end
	return ;
end
function postproc_file_Replace_c99_to_c98(szFilePath,avcodectbl)
	local avcodec_list_tbl=postproc_struct_postproc_get_table();
	--创建table表
	for index,value in pairs(avcodec_list_tbl) do
		avcodec_list_tbl[index].tbl=postproc_struct_postproc_default(szFilePath..value.file,value.name);
	end
		...
		if value~=nil then
			for fileindex,filename in pairs(value) do
				for list_index,list_value in pairs(avcodec_list_tbl) do
					...
					postproc_struct_postproc_Repalce(filename,list_value.tbl,list_value.name);
