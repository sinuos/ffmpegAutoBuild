...
--[[
函数:swresample_parser_allfile
功能:swresample 处理函数
参数:
返回:
]]--
function swresample_parser_allfile(szPath,includes,proType,isToc89)
	local filename="Makefile";
	local projName="libswresample";
	local makefiletabl={
	...
	for index,value in ipairs(makefiletabl) do
		...
		if(value.tbl~=nil) then
			...
			if isToc89 then
				swresample_file_Replace_c99_to_c98(szPath..value.path,avcodectbl);
			end;
		end;
	end
	return ;
end
function swresample_file_Replace_c99_to_c98(szFilePath,avcodectbl)
	local avcodec_list_tbl=swresample_struct_swresample_get_table();
	--创建table表
	for index,value in pairs(avcodec_list_tbl) do
		avcodec_list_tbl[index].tbl=swresample_struct_swresample_default(szFilePath..value.file,value.name);
	end
		...
		if value~=nil then
			for fileindex,filename in pairs(value) do
				for list_index,list_value in pairs(avcodec_list_tbl) do
					...
					swresample_struct_swresample_Repalce(filename,list_value.tbl,list_value.name);
